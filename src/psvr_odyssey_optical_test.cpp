...
int AutoCamera(const std::vector<PsvrCameraDeviceInfo> &devices)
{
  ...
  {
    const auto n = Lower(d.name);
    if (n.find(L"playstation") != std::wstring::npos || n.find(L"ps4") != std::wstring::npos ||
...
int main(int argc, char **argv)
{
  ...
  const auto devices = PsvrCameraCapture::Enumerate();
  ...
  if (devices.empty())
  {
    std::fprintf(stderr, "No Media Foundation camera device found. Install/enable the Windows PS Camera driver first.\n");
    return 2;
  }
