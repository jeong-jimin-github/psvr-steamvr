...
void MadgwickAhrs::BlendMagYaw(float mx, float my, float mz, float alpha)
{
  const float m2 = mx * mx + my * my + mz * mz;
  if (m2 < 1.f || alpha <= 0.f)
    return;

  const Quaternion q = q_;
  const float tx = 2.f * (q.y * mz - q.z * my);
  const float ty = 2.f * (q.z * mx - q.x * mz);
  const float tz = 2.f * (q.x * my - q.y * mx);
  const float wx = mx + q.w * tx + (q.y * tz - q.z * ty);
  const float wz = mz + q.w * tz + (q.x * ty - q.y * tx);
  const float mag_yaw = std::atan2(wx, wz);

  const float fx = 2.f * (q.x * q.z + q.w * q.y);
  const float fz = 1.f - 2.f * (q.x * q.x + q.y * q.y);
  const float imu_yaw = std::atan2(fx, -fz);

  float err = mag_yaw - imu_yaw;
  while (err > 3.14159265f)
    err -= 6.2831853f;
  while (err < -3.14159265f)
    err += 6.2831853f;

  const float half = 0.5f * alpha * err;
  const float cw = std::cos(half);
  const float cy = std::sin(half);
  Quaternion yq{cw, 0.f, cy, 0.f};
  Quaternion o;
  o.w = yq.w * q.w - yq.y * q.y;
  o.x = yq.w * q.x + yq.y * q.z;
  o.y = yq.w * q.y + yq.y * q.w;
  o.z = yq.w * q.z - yq.y * q.x;
  const float n = 1.f / std::sqrt(o.w * o.w + o.x * o.x + o.y * o.y + o.z * o.z);
  q_ = {o.w * n, o.x * n, o.y * n, o.z * n};
}
